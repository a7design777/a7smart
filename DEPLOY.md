...
> **Порядок має значення.** Спершу дочекайтеся, поки збірка в
> [Actions](https://github.com/a7design777/a7smart/actions) стане зеленою, і лише
> потім робіть `pull`. Інакше заберете попередній `latest` — `docker pull`
> зробить це мовчки, без жодної помилки.
>
> Що саме розгорнуто, показує `version` у `/api/health` — це SHA коміту.
> Звіряйте його з останнім комітом у `main`, перш ніж шукати причини,
> чому змін «не видно».

Перевірити версію після оновлення:
...
```bash
git -C /root/a7smart log -1 --format=%H
```
