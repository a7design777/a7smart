...
> потім робіть `pull`. Інакше заберете попередній `latest`, і якщо в оновленні
> була міграція, що змінює схему, старий код працюватиме з новою базою.
>
> Симптом саме такого розсинхрону: `/api/health` показує
> `1106 permission deny` від Tuya, а `lastPollAt` лишається `null`.
> Лікується повторним `pull` після завершення збірки.
